...
class ReplayMemory(object):
    ...
    def __init__(self, capacity,batch_size, device):
        self.framebuffer = deque(maxlen = 5)
        self.currentstate = deque(maxlen = 4)
        for i in range(5):
            self.framebuffer.append(torch.zeros((1, 84, 84)))
        for i in range(4):
            self.currentstate.append(torch.zeros((1, 84, 84)))
    ...
    def push(self, state, action, reward, done):
        ...
        self.framebuffer.append(state.view(1, 84, 84))
        transition = (torch.cat(list(self.framebuffer))[0:4].detach().numpy(), action, reward, torch.cat(list(self.framebuffer))[1:5].detach().numpy(), done)
    ...
    def sample(self):
        ...
        #print(transitions)
        #state, reward, action, next_state, done = zip(*transitions)
        #print(state)
        #print(torch.stack(list(state), dim = 0).shape)
...
class DQN:
    def __init__(self, args):
        ...
        self._memory = ReplayMemory(capacity=args.capacity, batch_size=args.batch_size, device=args.device)
    ...
    def select_action(self, state, epsilon, action_space):
        ...
        ## TODO ##
        state = torch.tensor(state, dtype=torch.float)
        self._memory.currentstate.append(state.view(1, 84, 84))
        state = torch.cat(list(self._memory.currentstate)).unsqueeze(0).to(self.device)
        #print(torch.cat(list(self._memory.currentstate)).shape)
        if random.random() < epsilon:
            action = action_space.sample()
        else:
            action = torch.argmax(self._behavior_net(state)).item()
        return action
    ...
    def _update_behavior_network(self, gamma):
        ...
        with torch.no_grad():
            q_next = torch.max(self._target_net(next_state), dim=1)[0].view(self.batch_size,-1)
...
def train(args, agent, writer):
    ...
    for episode in range(args.episode):
        ...
        for t in itertools.count(start=1):
            ...
            ## TODO ##
            # store transition
            agent.append(state, action, reward, done)
