...
class DQN:
    def __init__(self, args):
        self._behavior_net = Net(hidden_dim=128).to(args.device)
        self._target_net = Net(hidden_dim=128).to(args.device)
    ...
    def _update_behavior_network(self, gamma):
        ...
        with torch.no_grad():
            next_action = torch.max(self._behavior_net(next_state), dim=1)[1].view(self.batch_size,-1)
    ...
    for n_episode, seed in enumerate(seeds):
        ...
        for t in itertools.count(start=1):
            ...
            if done:
                ...
                print(
                    'Episode {}: {:.2f}'.format(n_episode, total_reward))
                break
        ## TODO ##
        # ...
        #     if done:
