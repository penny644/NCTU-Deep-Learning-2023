...
class time_embedding(nn.Module):
    ...
    def forward(self, x):
        half = self.output_channel // 2
        freqs = torch.exp(-math.log(self.total_timestep) * torch.arange(start=0, end=half, dtype=torch.float32) / half).to('cuda:0')
...
def ddpm_schedules(beta_start = 0.0001, beta_end = 0.02, total_timesteps = 300, device = 'cuda:3'):
    
    beta_t = (torch.linspace(beta_start**0.5, beta_end**0.5, total_timesteps+1) ** 2).to(device)
    alpha_t = 1 - beta_t
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_result_path', default='./result_noise/', type=str)
