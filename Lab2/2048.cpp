...
class board {
public:
	board(uint64_t raw = 0) : raw(raw) {}	//set all value in board empty
	 ...
	 * get a 16-bit row
	 */
	int  fetch(int i) const { return ((raw >> (i << 4)) & 0xffff); }	//每次raw向右移i*16，因為每四位是board的一個數字，取出最後十六位(跟0xffff and)
	/**
	 * set a 16-bit row
	 */
	void place(int i, int r) { raw = (raw & ~(0xffffULL << (i << 4))) | (uint64_t(r & 0xffff) << (i << 4)); }	//0xffff都是1向左移i*16，變成0xffff0000...，接著not變成0x00001111接著and就會把raw某一個row變為0再跟r or，就把那列變成r了
	/**
	 * get a 4-bit tile
	 */
	int  at(int i) const { return (raw >> (i << 2)) & 0x0f; }	//每次raw向右移i*4，因為每四位是board的一個數字，取出最後四位(跟0x0f and)，可依序取出board的每一位
	/**
	 * set a 4-bit tile
	 */
	void set(int i, int t) { raw = (raw & ~(0x0fULL << (i << 2))) | (uint64_t(t & 0x0f) << (i << 2)); }	//0x0f向左移i*16，變成0x0f0...，接著not變成0xf0f接著and就會把raw某一個row變為0再跟r or，就把那個位置變成r了
...
private:
	...
	struct lookup {
		...
		void init(int r) {
			...
			int V[4] = { (r >> 0) & 0x0f, (r >> 4) & 0x0f, (r >> 8) & 0x0f, (r >> 12) & 0x0f };	//取idx 0,1,2,3這四個位置的值
			...
			score = mvleft(R); std::reverse(R, R + 4);	//把R翻轉成跟L一樣
		...
		static int mvleft(int row[]) {	//往左移的動作
			int top = 0;
			int tmp = 0;
			int score = 0;

			for (int i = 0; i < 4; i++) {
				int tile = row[i];	//tile是某一個位置的值
				if (tile == 0) continue;	//如果是0不受任何動作影響
				row[i] = 0;	//把原本那格變為0
				if (tmp != 0) {
					if (tile == tmp) {
						tile = tile + 1;	//把一樣的組合起來
						row[top++] = tile;
						score += (1 << tile);
						tmp = 0;
					} else {
		...
		static const lookup& find(int row) {
			static const lookup cache[65536];	//因為每個feature有16^4=65536
			return cache[row];		//取一個row
		}
	};
...
public:
	...
	void popup() {
		int space[16], num = 0;
		for (int i = 0; i < 16; i++)
			if (at(i) == 0) { //board has function at(i)->可依序取出board上每一個位置
				space[num++] = i; //把board上空的位置的index存進space
			}
		if (num)
			set(space[rand() % num], rand() % 10 ? 1 : 2);	//rand%num隨機決定要放在哪個位置，後面表示有0~9都填入2只有10的倍數填入4->0.9為2，0.1為4
	}
	...
	int move_left() {	//每一列依序往左
		uint64_t move = 0;
		uint64_t prev = raw;
		int score = 0;
		lookup::find(fetch(0)).move_left(move, score, 0);	//把結果存在move裡，(raw, score, 第幾列)
		...
		raw = move;
		return (move != prev) ? score : -1;	//如果不相等就return得分，相等表示已無空格，表輸掉
	}
	int move_right() {
		uint64_t move = 0;
		uint64_t prev = raw;
...
public:
	pattern(const std::vector<int>& p, int iso = 8) : feature(1 << (p.size() * 4)), iso_last(iso) {
		...
		for (int i = 0; i < 8; i++) {
			...
			idx.rotate(i);
			for (int t : p) {
				isomorphic[i].push_back(idx.at(t));	//找pattern p裡面所有的值對應位置現在的idx，組成isomorphic
			}
		}
	}
...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				// TODO
				board after_find_empty = move->after_state();
				int space[16], num = 0;
				for (int i = 0; i < 16; i++){
					if (after_find_empty.at(i) == 0) { //board has function at(i)->可依序取出board上每一個位置
						space[num++] = i; //把board上空的位置的index存進space
					}
				}
				float after_value = 0;
        		for (int i = 0; i < num; i++){
					board tmp = after_find_empty;
					tmp.set(space[i], 1);
					after_value += 0.9 * estimate(tmp);
					tmp = after_find_empty;
					tmp.set(space[i], 2);
					after_value += 0.1 * estimate(tmp);
				}
				after_value = after_value / num;
				move->set_value(move->reward() + after_value);
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		float TD_error = 0;
		float after_state_v = 0;
		for(int i = path.size()-1; i>=0 ; i--){
			TD_error = path[i].reward() + after_state_v - estimate(path[i].before_state());
			after_state_v = update(path[i].before_state(), alpha*TD_error);
		}
	}
...
int main(int argc, const char* argv[]) {
	...
	tdl.add_feature(new pattern({ 1, 2, 5, 6}));
 	tdl.add_feature(new pattern({ 2, 6, 9, 10}));
	tdl.add_feature(new pattern({ 1, 0, 5}));
	
	// restore the model from file
	tdl.load("weight_best.bin");
