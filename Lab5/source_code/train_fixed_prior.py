...
from utils import init_weights, kl_criterion, plot_pred, finn_eval_seq, pred, plot_loss, plot_psnr
...
def train(x, cond, modules, optimizer, kl_anneal, args, device):
    ...
    for i in range(1, args.n_past + args.n_future):
        ...
        frame_pred_input = torch.cat([h_t_1, z_t, action_and_position], 1)
        g_t = modules['frame_predictor'](frame_pred_input)

        x_t_hat = modules['decoder']([g_t, skip])
        
        mse += mse_criterion(x_t_hat, x[i])
...
class kl_annealing():
    ...
    def update(self):
        if(self.kl_anneal_cyclical):
            period = int(self.niter / self.kl_anneal_cycle)
            beta_add = (1 - 0) / (period * self.kl_anneal_ratio)
        else:
            period = self.niter
            beta_add = (1 - 0) / (period * self.kl_anneal_ratio)
        if((self.epoch % period) < (period * self.kl_anneal_ratio)):
            if(self.epoch % period == 0):
                self.beta = 0
            else:
                self.beta = self.beta + beta_add
        else:
            self.beta = 1
        self.epoch += 1
...
def main():
    ...
    for epoch in range(start_epoch, start_epoch + niter):
        ...
        kl_anneal.update()
        for _ in trange(args.epoch_size):
            try:
        ...
        mse_all.append(epoch_mse / args.epoch_size)
        kld_all.append(epoch_kld / args.epoch_size)
        total_loss_all.append(epoch_loss  / args.epoch_size)
        tfr_all.append(args.tfr)
        beta_all.append(kl_anneal.get_beta())


        if epoch >= args.tfr_start_decay_epoch:
            ### Update teacher forcing ratio ###
            if((args.tfr - args.tfr_decay_step) > args.tfr_lower_bound):
    ...
    plot_loss(kld_all, mse_all, total_loss_all, tfr_all, beta_all, args)
    plot_psnr(PSNR_all, args)
